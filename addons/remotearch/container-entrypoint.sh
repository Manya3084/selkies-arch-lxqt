...
mkdir -pm1777 /dev/input || sudo mkdir -pm1777 /dev/input || echo 'Failed to create joystick interposer device directory'
...
make_node() {
  mknod "$1" c "$2" "$3" || sudo mknod "$1" c "$2" "$3" || echo "Failed to create device file $1"
}
...
if [ -d /dev/input ]; then
  ...
  chmod 0666 /dev/input/js* /dev/input/event* ||
    sudo chmod 0666 /dev/input/js* /dev/input/event* ||
...
drop_service() {
  ...
  { : > "/etc/service/$1/down"; } 2>/dev/null ||
    sudo sh -c ": > '/etc/service/$1/down'" 2>/dev/null || true
}

for service in /etc/service/*/; do
  rm -f "${service}down" 2>/dev/null || sudo rm -f "${service}down" 2>/dev/null || true
done
